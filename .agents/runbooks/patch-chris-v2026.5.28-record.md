...
- 频道探测(restart 后):Feishu(`<account-label>`)、iMessage、Telegram `<bot-label-1>`、Telegram `<bot-label-2>` 全 `works`
...
**核对**:`openclaw status` → Gateway app **2026.5.28**,pid 34211,state active,reachable 52ms,auth OK(`<gateway-host> <lan-ip>`)。config 迁移干净:版本戳(5.27→5.28、5.20→5.28)+ voice key 重命名 `voiceId`→`speakerVoiceId`(值保留)。**无多 provider 链改动**。
