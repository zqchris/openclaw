...
- Post-restart verification at `2026-05-22T11:43:52+08:00`:
  ...
  - Channel probes `ok: true` for imessage, feishu (`<group-label>`), telegram (`<bot-handle>`).
...
- 2026-05-21 morning: 6 imessage provider auto-restarts in 75 min (09:06
  ...
  silence.
- 2026-05-22 04:22: `<private-sender>` sent `<private-imessage-request>` via
  iMessage. social agent session `<session-id>` received it, ran cron tools,
  produced final assistant text `<private-assistant-confirmation>` at 04:27:10
  — **never delivered**. Subsequent sessions (`<session-id>` at 09:13)
...
- Write a launchd watcher that re-runs `imsg launch` whenever a fresh
  ...
  The operator explicitly deferred this — accepted the manual `imsg launch`
...
- The transport=bridge config change (commit `88e590ff53`) requires a
  **second** gateway restart to load the new dist; pending the operator's
  decision on timing.
...
- The Feishu attachment resolver (`d7eb9586bf`) still has the three known
  ...
  images are not picked up by `parseFeishuMediaKeys`. The operator confirmed
